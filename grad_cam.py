...
def get_gradcam(batch_img, batch_label, modelPath, dan, tf_sess, img_mask=1, img_size=IMGSIZE, conv_layer='S2_Conv4a', logging=False):
...
def visualize(batch_img, batch_label, modelPath, dan, sess, img_mask=1, batch_size=1, img_size=224, alpha=0.5, conv_layer='S2_Conv4b', vis_type='overlay', outFile=None):
    """Plot GradCam visualization for given image"""
    cam = get_gradcam(batch_img, batch_label, modelPath, dan, sess, img_mask, img_size, conv_layer, logging=True)
...
def get_most_activated_landmarks(cam, gt_landmarks, k, radius=3, img_size=224):
